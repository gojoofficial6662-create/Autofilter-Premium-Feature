...
API_ID = int(environ.get('API_ID', '36956279')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', '026d4ef35856ac3a8b6c5236b06035fe')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "8605781722:AAE85DKqAaLqgAcyyEERJqDC_GHH8XsdAQk")    # Bot token from @BotFather
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '8516071734').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003527036670').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1003999542749'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1003950093536'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1003739193452'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1003732595057').split()] #(make sure bot is admin)
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-100')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1003664584698')  # Request channel id (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Gojoofficial6969@cluster0.tb5l339.mongodb.net")  # MongoDB URI for the database
...
LOG_VR_CHANNEL = int(environ.get('LOG_VR_CHANNEL', '-1003984321136')) #Verification Channel Id 
LOG_API_CHANNEL = int(environ.get('LOG_API_CHANNEL', '-1003950332029')) #If Anyone Set Your Bot In Any Group And Set Shortner In That Group Then In This Channel The All Details Come
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/aklalakaka') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/nakalaoaoaoaooak') # Owner link for the bot
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', 'https://t.me/updatechannel23') # Update channel link for the bot
